...
class Date:
    ...
    def input_date(self) -> None:
        'date entry'
        while True:
            date = input(ColorEnum.GREEN + "Введите дату в формате День.Месяц.Год(если дата до н.э., то вводите -Год):  " + ColorEnum.FINISH)
            try:
                ...
                print(ColorEnum.RED + "Некорректный формат даты. Пожалуйста, введите дату в формате День.Месяц.Год." + ColorEnum.FINISH)
    ...
    def validate_date(self) -> None:
        ...
        if self.year == 0:
            raise DateValidationError(ColorEnum.RED + "Год 0 не существует (оказывается)" + ColorEnum.FINISH, 'invalid_year')
        if not 1 <= self.month <= 12:
            raise DateValidationError(ColorEnum.RED + "Некорректный месяц. Месяц должен быть от 1 до 12." + ColorEnum.FINISH, 'invalid_month')
        if not 1 <= self.day <= self.days_in_month():
            raise DateValidationError(ColorEnum.RED + "Некорректный день. Пожалуйста, проверьте количество дней в месяце." + ColorEnum.FINISH, 'invalid_day')
    ...
    def day_of_the_week(self) -> str:
        ...
        if sp_month < 3:
            sp_month += 12
            sp_year -= 1
        h = (self.day + ((13 * (sp_month + 1)) // 5) + sp_year + (sp_year // 4) - (sp_year // 100) + (sp_year // 400)) % 7
