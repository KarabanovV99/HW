...
@pytest.mark.parametrize("input_dict, expected_output", [
    ({'a': 1, 'b': 2}, '{"b": "2", "a": "1"}'),
    ({'x': 'hello', 'y': 'world'}, '{"x": "hello", "y": "world"}'),
    ({}, '{}')
])
def test_dict_to_hesh(input_dict, expected_output):
    hash_table = HashTable(len(input_dict), input_dict)
    assert str(hash_table) == expected_output
